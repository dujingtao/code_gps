...
- 深色 HUD／卫星雷达风格界面（Jetpack Compose）：
  - 顶部状态胶囊：搜索卫星中 / 已定位 / 权限被拒绝，搜索时状态点呈脉冲动画。
  - 中央雷达指示器（Canvas 绘制）：同心网格 + 搜索时的旋转扫描动画，外圈半径与颜色随 GPS 精度变化（越精确越小越绿，越差越大越黄/红）。
  - 半圆形速度表（Canvas 绘制的渐变弧线，0–30 m/s）。
  - 海拔、精度、最后更新时间以毛玻璃质感卡片展示（API 31+ 有真实背景模糊，以下版本自动降级为半透明纯色，不崩溃、不强制提高 `minSdk`）。
  - 所有数值变化都有滑入/淡入动画，不会生硬跳变。

## 技术栈

- 语言：Kotlin
- UI：Jetpack Compose + Material 3，自定义 Canvas 绘制（雷达指示器、速度表）
...
├── MainActivity.kt                    # 入口 Activity：权限请求 + edge-to-edge + 生命周期感知的位置订阅
├── location/
...
└── ui/
    ...
    └── components/
        ...
        ├── RadarIndicator.kt            # 雷达/精度圈 HUD 指示器
        ├── SpeedGauge.kt                # 半圆速度表
